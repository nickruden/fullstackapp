<template>
  <div>
    <router-link :to="{ name: 'Create' }" class="button is-success mt-6"
      >Add New</router-link>
...
export default {
  ...
  methods: { 
    ...
    async getProducts() {
      try {
        const response = await axios.get("https://fullstackapp-api.vercel.app/products");
    ...
    async deleteProduct(id) {
      try {
        await axios.delete(`https://fullstackapp-api.vercel.app/products/${id}`);
        this.getProducts();
      } catch (err) {
        console.log(err);
      }
